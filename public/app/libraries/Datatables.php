  ...
  class Datatables
  {
    ...
    */
    private function get_filtering()
    {
        ...
        for($i = 0; $i < count($mColArray); $i++){
          if($mColArray[$i]['searchable'] == 'true' ){
            ...
            if($mColArray[$i]['search']['value']){
              ...
              if($this->columns[$i]['search']['value']){
                $sWhere .= $this->select[$this->columns[$i]] . " LIKE '" . $this->columns[$i]['search']['value'] . "%' AND ";
              }
            }
          }
        }
      ...
      $sWhere = preg_replace("/(AND|OR)\s?$/i", "", $sWhere);
    ...
    */
    private function get_total_results($filtering = FALSE)
    {
      ...
      $num_rows = $this->ci->db->count_all_results($this->table);
      return $num_rows;
    }
